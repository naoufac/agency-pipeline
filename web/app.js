...
function roadmap(){
  const P = [
    ...
    { n:'PQ1', t:'Distinct design per brief', s:'next', d:'Two different businesses must NOT look like the same page recolored. Real compositional variety: multiple hero treatments (full-bleed, split, editorial, product-first), nav styles, section rhythms and layouts chosen from the brief — not just swapped fonts and colours. Gate: an automated check that N briefs produce structurally different layouts, plus a design critique that passes.' },
    { n:'PQ2', t:'Ecommerce that actually sells', s:'next', d:'Real store primitives that do not exist today: a product-detail view (one product, price, options, add-to-cart), a working cart (line items, quantities, total, remove), and a checkout that reads the cart and writes a real order row. Gate: a browser adds two products, changes a quantity, checks out, and the order + line items land in the database.' },
